...
class CheckStatusTest extends CIUnitTestCase
{
    ...
    public function testCheckStatusFound(): void
    {
        ...
        $orderModel->insert([
            ...
            'status' => 'menunggu_pembayaran', 'public_access_token' => $token,
        ]);
        $categoryModel->db->query('PRAGMA foreign_keys = ON');

        $result = $this->call('post', 'cek-pesanan', [csrf_token() => csrf_hash(), 'invoice_number' => 'INV99999', 'access_token' => $token]);
        $result->assertRedirectTo('/pesanan/INV99999/' . $token);
        $invoice = $this->get('/pesanan/INV99999/' . $token);
        $invoice->assertOK();
        ...
        $this->get('/pesanan/INV99999/' . str_repeat('b', 64));
    }
